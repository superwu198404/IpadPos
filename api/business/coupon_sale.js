...
export default {
	methods: {
		coupons_segment_repeat_judge(start_and_end,
			start_and_end_list) { //param 1：券起始号码-券结束号码，param 2：[券起始号码 A-券结束号码 A,券起始号码 B-券结束号码 B,...]
			...
			if (input_arr?.length == 2) {
				...
				for (let s_e of start_and_end_list) {
					...
					if (s_e_arr?.length == 2) {
						...
						if (start_code <= start_code_input && end_code >= start_code_input || start_code <=
							end_code_input && end_code >= end_code_input) {
							return true;
						}
					}
				}
		...
		coupon_price_record(sale2, sale6) {
			console.log("[CouponPriceRecord]券金额记录:", {
				sale2,
				sale6
			});
			sale2.forEach(s2 => {
			...
			console.log("[CouponPriceRecord]券金额处理后的记录:", {
				sale2,
				sale6
			});
		}
