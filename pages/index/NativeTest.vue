		...
		<view>
			...
			<ul id="li2">
				<li v-for="(item, index) in vlist2" :key="index">
					<text v-if="item.flag=='searchDevices'" @click="searchDevices(item.id)">id:{{item.id}},name:{{item.name}}</text>
					...
					onReceive: function(context, intent) { //实现onReceiver回调函数  
						...
						if (intent.getAction() == "android.bluetooth.adapter.action.DISCOVERY_FINISHED") {
							...
							if (BleDevice.getBondState() == bdevice.BOND_NONE) {
								console.info("未配对蓝牙设备：" + BleDevice.getName() + '    ' + BleDevice.getAddress());
								console.info("address：" ,address ,'   ' + BleDevice.getAddress());
								//参数如果跟取得的mac地址一样就配对  
								if (address == BleDevice.getAddress()) {
									if (BleDevice.createBond()) { //配对命令.createBond()  
										console.info("配对成功");
										let temp={id: BleDevice.getAddress(),flag: 'print',name: BleDevice.getName()};
									...
									if (BleDevice.getName() != on) { //判断防止重复添加   
									 ...
									 let temp={id: BleDevice.getAddress(),flag: 'searchDevices',name: on};
