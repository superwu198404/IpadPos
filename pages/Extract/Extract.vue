			...
			<view class="bkjb">
				<view class="neik">
					<view class="commodity" style="height: 100%;">
						<view class="hh">
							<view class="hotcakes">
								<image src="../../images/ydtq.png" mode="widthFix"></image>
								{{ view.mode ? '预定提取' : '预定取消'}}
									...
									<view class="sousuo">
										...
										<view class="criterias" v-if="view.Criterias">
											<view class="critlist"><text>订单号：</text><input type="text"
													v-model="condition.bill" />
											</view>
											<view class="critlist"><text>手机号：</text><input type="text"
													v-model="condition.phone" />
											</view>
											<view class="critlist"><text>提货方式：</text>
												<picker class="extract-type" @change="ExtractType"
													:range="extract_types" range-key="NAME" value="index">
													<view>
														{{ condition.type ? (extract_types.length > condition.type ? extract_types[condition.type].NAME : "") : "" }}
													</view>
												</picker>
											</view>
							...
							<view class="procycle">
								...
								<view v-for="(item,index) in extracts" class="li">
									<view class="title-box">
										...
										<view class="price" style="font-weight: 400;font-size:24rpx;">
											{{ item.THDATE || '-' }}
										</view>
									</view>
									<view class="cods">
									...
									<view class="handles"><text>配送地址:{{ item.CUSTMADDRESS || ' -' }}</text>
										...
										<button @click="ExtractOrder(item)" class="btn"
											:disabled="view.loading === item.BILL">{{ view.mode ? '提取' : '取消'}}
										</button>
									</view>
								</view>
							</view>
	...
	export default {
		...
		methods: {
			...
			EditOrder: function(item) {
				...
				RequestSend(
					`select  1  from  ywbhqh where khid='${item.KHID}' and  Bill_Status ='1' and Bill_Yd ='${item.BILL}'`,
