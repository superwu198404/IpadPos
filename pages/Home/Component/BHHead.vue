		...
		<view class="nav" style="display: flex;justify-content: space-between;align-items: center;position: relative;">
			...
			<view class="stores" style="position: absolute;right: 1%;">
				<view class="checkout">
					...
					<!-- 	<label>
						<image src="../../../images/imgbh/bhjian.png" mode="widthFix"></image>{{STORE_NAME}}
					</label> -->
					<label>
	...
	export default {
		...
		data() {
			return {
				...
				ynDKF: true,
				YN_SX: false, //是否赊销
				timeTick: 0, // 触发computed更新的响应式变量
				timer: null, // 定时器ID
				currentTime: '11' // 直接存储格式化后的时间
			};
		},
		computed: {
			custom_display() {
		...
		methods: {
			...
			updateTime() {
				...
				// 直接修改 data 中的 currentTime，强制更新视图
				this.currentTime = `${year}年${month}月${day}日 ${week} ${hours}:${minutes}:${seconds}`;
			},
			// clearTime 保留不变
			clearTime() {
				if (this.timer) {
