...
var jpPrinter = {
	createNew: function() {
		...
		jpPrinter.byteToString = function(arr) {
			...
			console.log('byteToString 111', str)
			return str;
		};
		...
		jpPrinter.setTextNew = function(content) { //设置文本内容
			data.push(content)
		}

		jpPrinter.initNew = function() { //初始化打印机
			var Print = [];
			Print.push(27)
			Print.push(64)
		...
		jpPrinter.setBarcodeWidthNew = function(width) { //设置条码宽度
			var Print = [];
			Print.push(29)
			Print.push(119)
			if (width > 6) {
		...
		jpPrinter.setBarcodeHeightNew = function(height) { //设置条码高度
			data.push(29)
			data.push(104)
			data.push(height)
		}

		jpPrinter.setBarcodeContentNew = function(content) {
			var code = new encode.TextEncoder(
				'gb18030', {
					NONSTANDARD_allowLegacyEncoding: true
				}).encode(content)
		...
		jpPrinter.setPrintNew = function() { //打印并换行
			var Print = [];
			Print.push(10)
			var strPrint = jpPrinter.byteToString(Print);
			data.push(strPrint)
		...
		jpPrinter.setPrintBarcodeNew = function(m) { //打印条码并换行
			var Print = [];
			Print.push(29)
			Print.push(107)
			Print.push(m)
		...
		jpPrinter.setPrintAndFeedNew = function(feed) { //打印并走纸feed个单位
			var Print = [];
			Print.push(27)
			Print.push(74)
			Print.push(feed)
		...
		jpPrinter.setPrintAndFeedRowNew = function(row) { //打印并走纸row行
			var Print = [];
			Print.push(27)
			Print.push(100)
			Print.push(row)
		...
		//大号字体
		jpPrinter.Size2 = function(n) {
			console.log('Size2 111')
			data.push(29)
			data.push(33)
			data.push(n)
		};
		...
		//大号字体
		jpPrinter.Size2New = function(n) {
			console.log('Size2 111')
			var Size2 = [];
			Size2.push(29)
			Size2.push(33)
			Size2.push(n)
			var strSize2 = jpPrinter.byteToString(Size2);
			data.push(strSize2)
		};
