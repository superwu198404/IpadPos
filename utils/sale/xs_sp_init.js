	...
	*/
	arrListGroupBy: function(pm_arr, cake_arr, drinkP_arr) {
		...
		retArr = pm_arr.sort((a, b) => {
			let x = 0;
			if ((a.FSTR < b.FSTR) || (a.FSTR == b.FSTR && a.plid < b.plid)) {
				x = -1
			} else {
				x = 1
			}
		...
		retArr.forEach(item => {
			...
			if (item.ynshowlist == "1") {
				item.specslist = cake_arr.filter(cakeitem => {
					return cakeitem.SPID == item.SPID
				})
				console.log(JSON.stringify(item.specslist));
			}
			if (item.ynAddPro == "1") {
				item.addlist = drinkP_arr.filter(drinkitem => {
	...
	loadSp: async function(pm_storeid, callbackfun, pm_dqid) {
		...
		let drinkProSql = " SELECT  MATNR, CSTCODE,ATTCODE, ATTNAME, OPTCODE, OPTMAT, OPTNAME, RECMARK  " +
			" FROM  KXPSX   WHERE   ifnull(DELMK,'N')='N' AND    BZIRK='" + pm_dqid + "'" +
