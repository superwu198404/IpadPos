...
var XsTypeObj = {
	...
	sale_credit: {
		...
		CloseBigCustomerJGZ: function(dkfdata,data) {
			...
			if (this.clickSaleType.clickType == 'sale_credit' && this.DKF.cval.DKFID != '80000000' && Object
				.keys(dkfdata).length == 0) { //赊销打开大客户后 没选择大客户则不做更改
				return;
			}
			this.SXJGZ = data;
	...
	sale_credit_return_good: {
		...
		$initSale: function(params) { //清除一下会员信息
			...
			console.log("[sale_credit_return_good]SXSALE003:", this.sxsale001);
...
function GetSale(global, vue, target_name, uni) {
	...
	//*func*特殊折扣关闭回调
	this.CloseTSZK = util.callBind(this, function(type, data) {
		this.setComponentsManage(null, "Disc");
		...
		if (type == "NO") { //清除折扣
			...
			else if (type == 'HIS' && data && data.length > 0) //历史折扣时才赋值
			{
				obj.ZKData_HIS = data; //选择的历史折扣数据
				obj.ZKData_New = []; //清空一下呢
			} else {
				obj.ZKData_HIS = []; //清空一下呢
				obj.ZKData_New = []; //清空一下呢
			}
	...
	const handleClassifySearch = function(object, searchKey) {
		const result = []
	...
	const handleNoClassifySearch = function(object, searchKey) {
		const result = []
		let list = []
		for (const key in object) {
			if (object.hasOwnProperty(key)) {
	...
	this.queryBoardList = function(searchKey) {
		const classDate = handleClassifySearch(that.spidKeyVal, searchKey)
		const noClassDate = handleNoClassifySearch(that.spidKeyVal, searchKey)
		this.classifyDate = classDate
	...
	this.switchAreaChange = function(e) {
		if (e.detail.value) {
			that.isDateClassify = true
			that.selectFlagList = that.classifyDate
			if (that.selectFlagList && that.selectFlagList.length >= 1) {
				that.selectPlid = that.selectFlagList[0].plid
			}
			that.Page.$set(that.Page[that.pageName], "selectFlagList", that.selectFlagList);
		} else {
			that.isDateClassify = false
			that.selectFlagList = that.notClassifyDate
			that.Page.$set(that.Page[that.pageName], "selectFlagList", that.selectFlagList);
		}
	}
