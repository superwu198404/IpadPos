	...
	createNew: function() {
		...
		*/
		...
		jpPrinter.formString = function(data, printer_poscs, print) {
			...
			let isSB = false;
			try {
				let sbCode = data.goodsList.find(c => parseInt(c.spid).toString().substr(0, 3) == util.snvl(
					printer_poscs.SBLBBM, ""));
			...
			data.goodsList.forEach((item, i) => {
				...
				jpPrinter.setCharacterSize(0); //设置正常大小
				jpPrinter.setSelectJustification(0); //设置居左
				jpPrinter.setText(util.getComputedByteLen("", 15) + util.getComputedByteLen(item.qty
					.toString(), 6) + util.getComputedByteLen(item.originalNet.toString(),
					6) + util.getComputedByteLen(item.discount.toString(), 6));
				jpPrinter.setPrint(); //打印并换行		
			});
			...
			jpPrinter.setCharacterSize(0); //设置正常大小
			jpPrinter.setSelectJustification(0); //设置居左
			jpPrinter.setText("条目:" + lineNum.toString() + " 数量:" + data.totalQty.toString() + " 原金额:" +
				util.tnvl(data.originalAmount, 0));
			jpPrinter.setPrint(); //打印并换行
			...
			jpPrinter.setCharacterSize(0); //设置正常大小
			jpPrinter.setSelectJustification(0); //设置居左
			jpPrinter.setText("已优惠金额:" + util.tnvl(data.discountedAmount, 0) + " 应付金额:" + util.tnvl(data
				.payableAmount, 0));
			jpPrinter.setPrint(); //打印并换行
		...
		jpPrinter.SXFormString = function(data, printer_poscs, print) {
			...
			if (!isReturn) {
				...
				jpPrinter.setCharacterSize(0); //设置正常大小
				jpPrinter.setSelectJustification(0); //设置居左
				jpPrinter.setText("欠款:" + util.nnvl(data.payableAmount, 0).toFixed(2) + " 定金:" + util.nnvl(
					data.dnet, 0).toFixed(2));
				jpPrinter.setPrint(); //打印并换行
		...
		jpPrinter.FormStringWMYD = function(data, printer_poscs, print) {
			...
			if (data.update == "1" || data.update == "2") {
				jpPrinter.setCharacterScale(1); //设置放大一倍
				jpPrinter.setSelectJustification(0); //设置居左
				jpPrinter.setText("【订单变更】\n");
				jpPrinter.setPrint(); //打印并换行
			}
			let HYY = "";
			// 终端参数配置了欢迎语，则取配置
			...
			data.goodsList.forEach((item, i) => {
				...
				//  套餐商品 → 缩进两格==============================================
				if (item.isFirstComboItem && item.comboName && item.comboName !== null && item
					.comboName !== "") {
					jpPrinter.setCharacterSize(1); // 放大字体，实现加粗效果
					jpPrinter.setSelectJustification(0);
					jpPrinter.setText(item.comboName.toString());
					jpPrinter.setPrint();
					jpPrinter.setCharacterSize(0); // 恢复正常字体大小

				}
				if (item.isCombo) {
					spname = "  " + spname; // 套餐商品缩进
				}
				// ==============================================
				let isSpid = jpPrinter.spidSubStr(item.spid);
				if (isSpid && !isReturn) {
				...
				jpPrinter.setCharacterSize(0); //设置正常大小
				jpPrinter.setSelectJustification(0); //设置居左
				jpPrinter.setText(util.getComputedByteLen("", 15) + util.getComputedByteLen(item
						.pack.toString(), 6) + util
		...
		jpPrinter.FormStringWM = function(data, printer_poscs, print) {
			...
			goodsList.forEach((item, i) => {
				...
				jpPrinter.setText(util.getComputedByteLen("", 15) + util.getComputedByteLen(item.qty
						.toString(), 6) + util
					.getComputedByteLen(item.price.toString(), 6) + util.getComputedByteLen(item
						.net.toString(), 6) + util.getComputedByteLen(
						util.snvl(item.attribute, ""), 6));
				jpPrinter.setPrint(); //打印并换行		
			});
		...
		jpPrinter.FormStringXS = function(data, printer_poscs, print) {
			...
			jpPrinter.setCharacterSize(0); //设置正常大小
			jpPrinter.setSelectJustification(0); //设置居左
			jpPrinter.setText(util.getComputedByteLen(data.sname, 20) + util.getComputedByteLen(data.price
				.toString(), 8) + util.getComputedByteLen("1", 8));
			jpPrinter.setPrint(); //打印并换行
		...
		jpPrinter.formatString = function(data) {
			...
			strCmd += strLeft + strSize1 + (util.getComputedByteLen("款台: " + data.posId, 17) + "收银员: " +
				data.posUser) + strSetPrint;
			...
			data.goodsList.forEach((item, i) => {
				...
				strCmd += strLeft + strSize1 + (util.getComputedByteLen(item.spid, 15) + util
					.getComputedByteLen(item.qty.toString(), 6) + util
					.getComputedByteLen(item.price.toString(), 6) + util.getComputedByteLen(item
						.amount.toString(), 6) + util.getComputedByteLen(
						item.discount.toString(), 6)) + strSetPrint;
			});

			strCmd += strLeft + strSize1 + "--------------总计--------------" + strSetPrint;
			strCmd += strLeft + strSize1 + ("条目:" + lineNum.toString() + " 数量:" + data.totalQty.toString() +
				" 应付金额:" + data.payableAmount.toString()) + strSetPrint;
			strCmd += strLeft + strSize1 + ("已优惠金额:" + data.discountedAmount.toString() + " 原金额:" + data
				.originalAmount.toString()) + strSetPrint;
			...
			data.sale3List.forEach((item, i) => {
				...
				strCmd += strLeft + strSize1 + (item.fkName + ":" + item.amt.toString()) +
					strSetPrint;
		...
		jpPrinter.SkSqFormString = function(data, printer_poscs, print) {
			...
			if (type == "SKCZ" || type == "CZ" || type == "LPKJH" || type == "DHQHK") {
				jpPrinter.setCharacterSize(0); //设置正常大小
				jpPrinter.setSelectJustification(0); //设置居左
				jpPrinter.setText("成功:" + util.nnvl(data.sale6_sumQty, 0) + "张" + " 金额:" + util.tnvl(data
					.sale6_sumNet, 0));
				jpPrinter.setPrint(); //打印并换行

				jpPrinter.setCharacterSize(0); //设置正常大小
				jpPrinter.setSelectJustification(0); //设置居左
				jpPrinter.setText("失败:" + util.nnvl(data.failSumQty, 0) + "张" + " 金额:" + util.tnvl(data
					.failSumNet, 0));
				jpPrinter.setPrint(); //打印并换行

				jpPrinter.setCharacterSize(0); //设置正常大小
				jpPrinter.setSelectJustification(0); //设置居左
				jpPrinter.setText("已优惠金额:" + util.tnvl(data.discountedAmount, 0) + " 原金额:" + util.tnvl(data
					.originalAmount, 0));
				jpPrinter.setPrint(); //打印并换行
