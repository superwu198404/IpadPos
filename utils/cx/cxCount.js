...
const Cxdict = async () => {
	...
	for (let i = 0; i < dscxm.length; i++) {
		try {
			...
			C1.SDATE = cx_util.snvl(dscxm[i].SDATE, "");
			C1.EDATE = cx_util.snvl(dscxm[i].EDATE, "");
			try {
...
const Createcx = async (sale02_arr, xstype, hyinfoModel, ispoints) => {
	...
	for (let i = 0; i < sale02_arr.length; i++) {
		...
		let net = Math.round(parseFloat(sale02_arr[i].NET.toString()) * 100) / 100;

		//添加商品到促销计算数组
		AddRowCxbilldts(spid, price, num, net, i);
		//去计算促销
...
//向计算缓冲区添加一行
const AddRowCxbilldts = async (itemid, price, qty, net, row) => {
	if (qty == 0) {
		//删除
		CxdtReMoveRow(row);
	} else {
...
const SaleCxCreate = async (spid, bill, saledate, fxbill, hylevel) => {
	...
	for (let col = 0; col < cxbilldts.length; col++) {
		...
		for (let k = 11; k < cxbilldDataKeys.length; k++) {
...
const cxClasCompute = function(spid, salebill, saledate, bill, bufflist, sltype) {
	...
	try {
		var sdpriceMap = new Map();
		for (let col = 0; col < cxbilldts.length; col++) {
			let net = cx_util.nnvl(cxbilldts[col][NET], 0);
			sdpriceMap.set(Math.round((net + 0.00001 * (col + 1)) * 100000) / 100000, col);
		}

		let net_arr = Array.from(sdpriceMap)
		sdprice = []
		net_arr.forEach((item) => {
			sdprice.push({
				key: item[0],
				val: item[1]
			})
...
const JustOnelbcx = function(spid, bill, saledate, cx, pmList, qtytype) {
	try {
		...
		while (currentlv >= 0) {
			...
			//发生的总金额 或者总数量
			let Fsnet = 0;
			for (let p = 0; p < sdprice.length; p++) {
				let c = sdprice[p]["val"];
				for (let i = 0; i < pmList.length; i++) {
					if (i != c) {
						continue;
...
const AddCxTable = function(spid, bill, saledate, cx, subid, row, fsqty, newprice, price, level, PM_LCM, jfnum,
	...
	dr["OPRICE"] = price;
	dr["ONET"] = price * fsqty;
	dr["CXPRICE"] = Math.round(newprice * 100) / 100;
	dr["CXNET"] = Math.round((newprice * fsqty) * 100) / 100;
	dr["CXLV"] = level + 1;
	dr["LCM"] = PM_LCM;
	dr[hylv] = cx.HYLV;
	dr["SPJF"] = jfnum;
